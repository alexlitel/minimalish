    ...
    (function mobileToggle() {
        $('#menu-toggle').on('click touchstart', function(event) {
            ...
            if (!($(this).hasClass('is-rotated'))) {
                ...
                $(this).attr({
                    "title": "Click, tap, or press 'Enter' to hide menu.",
                    "aria-expanded": "true"
                });
                ...
                $(this).attr({
                    'title': "Click, tap, or press 'Enter' to show menu.",
                    "aria-expanded": "false"
                });
            }
        ...
        $('#search-toggle').on('click touchstart', function(event) {
            ...
            if (!($(this).hasClass('tumblr-icon-cancel'))) {
                ...
                $(this).attr({
                    'title': "Click, tap, or press 'Enter' to hide search.",
                    'aria-expanded': "true"
                });
                ...
                $(this).attr({
                    'title': "Click, tap, or press 'Enter' to show search.",
                    'aria-expanded': "false"
                });
            }
        ...
        //Refocus on icon after cycling through header links w/ tab
        $('.header-box-link-list-item:last-child').on('keydown', function(event) {
            if ($('#menu-toggle').css('display') === 'block') {
                var code = event.which || event.keycode;
                //make sure not shift+tab
        ...
        //Refocus on icon after cycling through search options w/ tab
        $('.header-box-submit').on('keydown', function(event) {
            if ($('#search-toggle').css('display') === 'block') {
                var code = event.which || event.keycode;
                //make sure keypress not shift+tab
                if (code === 9 && !event.shiftKey) {
                    event.preventDefault();
                    $('#search-toggle').focus();
                }
...
;(function() {
    ...
    Plugin.prototype = {
        ...
        _setupRows: function(elem, options) {
            ...
            if (options.layout) {
                ...
                for (var a = 0; a < $(elem).find('a').length; a++) {
            ...
            var $links = $(elem).find('.photoset-post-photoset-cell');
            var imageIndex = 0;
            ...
            $.each(this.rows, function(i, val) {
                ...
                $links.slice(rowStart, rowEnd).wrapAll('<div class="photoset-post-photoset-row photoset-post-photoset-row-cols-' + val + '"></div>');

                imageIndex = rowEnd;
            });

            // $(elem).find('.photoset-post-photoset-row:not(:last-child)').css({
        ...
        _setupColumns: function(elem, options) {
            ...
            var setupStyles = function(waitForImagesLoaded) {
                var $rows = $(elem).find('.photoset-post-photoset-row');
                var $links = $(elem).find('.photoset-post-photoset-cell');
                var $images = $(elem).find('img');
                ...
                // if the imaged did not have  /width attr set them
    ...
    //Setup photogrid
    $('.photoset-post-photoset').photosetGrid();
});
